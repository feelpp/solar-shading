...
echo "Enter the name of the .cpp file (without the .cpp extension), either uniform_int or uniform_real:"
read cpp_file_name
cpp_file_name="src/$cpp_file_name"
...
do
    ...
    g++ -DFROM_SHELL_SCRIPT -Wno-enum-compare $flag -o $cpp_file_name $cpp_file_name.cpp
...
csv_file_name="$(realpath "$cpp_file_name" | sed -E 's|(.*/)(.*)|\1../../../results/csv/\2.csv|')"
echo "CSV file path: $csv_file_name"
...
case $answer in
    [Yy]* )
        head -n 1 "$csv_file_name" > temp.csv && mv temp.csv "$csv_file_name"
